...
public class OpenDiveSensor : MonoBehaviour {
	...
	void Update () {
		...
		if (add_rotation_gameobject){
			transform.rotation =Quaternion.Inverse(rotation_gameobject.transform.rotation* rot);
		}
		else
		{
			transform.rotation = rot;
			if (is_tablet==1)transform.rotation=Quaternion.Inverse(rot*Quaternion.AngleAxis(90,Vector3.forward));
